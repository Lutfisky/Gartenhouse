...
export class SensorSimulator {
  ...
  public async updateTableData(table: PlantTable, house: GreenhouseData) {
		...
		if (table.temperature < 5 || table.temperature > 40) {
			console.log(
				`⏸️ ${table.position} growth paused: ${table.temperature.toFixed(1)}°C outside 5-40°C`
			);
			return;
		}
		...
		if (table.soilMoisture >= 50 && table.soilMoisture <= 80) growth += 0.05;
		if (table.soilFertility >= 50) growth += 0.03;
		if (table.artLight > 500) growth += 0.05;
		...
		// 👉 LOGGING aller Werte
		console.log(
			`📊 ${table.position} | Size: ${table.plantSize.toFixed(2)}cm | ` +
